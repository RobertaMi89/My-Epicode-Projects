...
*/
if (pets.length > 1) {
  const firstPetItem = pets.indexOf(pets[0]); 
  if (firstPetItem !== -1) {
    const firstPet = pets.splice(firstPetItem, 1); 
    pets.push(firstPet[0]); 
  }
}
console.log(pets);
...
let licensePlateItem=158;
for(let i=0;i<cars.length;i++){
  cars[i].licensePlate= licensePlateItem
}
console.log(cars);
...
const newCar = {
  ...
  model: 'Yaris',
  color: 'Electric blue',
  trims: ['Ibrid','electric'],
};
cars.push(newCar);
const lastCar = cars[cars.length - 1];
if (lastCar.trims.length > 0) {
  lastCar.trims.pop();
}
console.log(cars);
...
for (let i = 0; i < charactersArray.length; i++) {
  ...
  const position = alfabeto.indexOf(char) + 1; 
